...
- プリフライトの通過条件は 1 つ——**キーが取得でき、対象モデル（指名がなければ環境で実際に配れるモデル）が `/v1/models` に載っていること。** 満たせなければ、そこで停止し、「結果: 失敗」として正確なエラーメッセージとともに即座に報告する。実装の代行はしない。
...
上記の `gpt-5.6-terra(high)` / `gpt-5.6-terra(medium)` / `gpt-5.4-mini` は執筆時点の一例である。契約プランによって `/v1/models` に見えるモデルは異なるため、実際に配れる GPT 系モデル ID に読み替えること。
...
| `--permission-mode acceptEdits` | 作業ツリー内のファイル編集を非対話で受理する。「作業ツリー限定」の思想を保つ（`bypassPermissions` / `--dangerously-skip-permissions` は使わない） |
...
| `--output-format stream-json --verbose` | セッションの全イベントを stream-json で観測可能にする（`stream-json` は `--verbose` を要する）。監視性がこのレーンの本命 |
...
`--model 'gpt-5.6-terra(high)'` は既定値であり定数ではない。統括者の指示が別の GPT モデルや effort を指名していればそちらを使う。

2. **最終メッセージを抽出する。** stream-json ログから `jq` で最終 result を取り出す。
