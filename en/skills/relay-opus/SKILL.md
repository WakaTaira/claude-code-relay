---
name: relay-opus
description: Skill for autonomously driving the implementation phase of tasks whose design is already agreed (Opus-orchestrator edition). Use when the main session model is Opus; use relay when it is fable. Fires when the user signals intent to execute an agreed plan ("go ahead", "start implementing", etc.) or explicitly invokes /relay-opus.
---
...
| relay-codex | sonnet / medium (implementation itself is GPT on a headless Claude Code via CLIProxyAPI) | Default lane for implementation tasks. Streams the settled spec to a headless Claude Code (GPT) and independently verifies the result |
